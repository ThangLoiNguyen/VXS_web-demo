...
export default function AboutPageContent() {
    ...
    return (
        <div className='flex flex-col w-full items-center py-10'>
            <h1 className='flex text-5xl font-bold pb-20'> We provide the web design</h1>
            <div className="flex flex-col gap-10">
                {dataAbout.map(({ id, title, text, image, background }) => (
                    <div key={id} className={`flex flex-row w-full gap-2
                        ...
                        <div className={`w-1/2 flex gap-10 p-3 items-center
                            ${background}
                            ${id % 2 === 1 ? 'flex-row rounded-r-full pl-10 animate-fade-left-right' : 'flex-row-reverse rounded-l-full pr-10 animate-fade-right-left'}`}>
                            <div className='flex flex-col items-center gap-3'>
                                <h3 className="text-3xl font-semibold">{title}</h3>
                                <p className="text-gray-600">{text}</p>
                            </div>
                            <Image src={image} alt={title} className={`w-1/2 object-cover
