...
import { faXmark, faPaperPlane, faEllipsis } from '@fortawesome/free-solid-svg-icons'
    ...
    return (
        <div>
            ...
            <div className='md:fixed z-50 h-0 bottom-20 right-10 animate-visible'>
                ...
                <div className='hover:bg-[--primary-color] p-3 bg-sky-600 rounded-full cursor-pointer duration-300'>
                        <FontAwesomeIcon icon={faFacebookMessenger} 
                        onClick={handleShowChat}
                        className='h-6 w-6 object-cover text-white duration-300' />
                </div>
                <span className='absolute -z-10 top-[6px] left-[6px] rounded-full w-9 h-9 bg-red-600 animate-ping'></span>
                {showChat && 
                <div className='absolute border border-sky-600 -bottom-14 right-16 w-[400px] h-[500px] bg-white rounded-2xl select-none overflow-hidden animate-fade-in'>
                    <div className='relative h-16 w-full bg-sky-600'>
                        <img src="vxs-logo.jpg" alt=" vxs-logo" className='absolute top-1/2 -translate-y-1/2 left-3 h-10 w-10 rounded-full object-cover' />
                    ...
                    <div className='absolute flex justify-center bottom-3 left-0 w-full h-12'>
                        <input type="text" placeholder='Aa' className='flex -translate-x-5 w-[80%] h-full rounded-full px-5 placeholder-gray-500 bg-gray-200 outline-none text-black' />
